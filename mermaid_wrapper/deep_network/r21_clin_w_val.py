    ...
    def test_model(self):
        ...
        with torch.no_grad():
            for j, images in enumerate(self.test_data_loader, 0):
                ...
                cb_labels_name = os.path.join('../', self.img_list[j].split(' ')[5].replace('SmBowel_label', 'oar_label'))

                #if not ("OG" in ct_image_name and "OG" in cb_image_name):
                #     print("Simulated case, ignore")
                #     continue
                patient = ct_image_name.split('/')[3]
                #patient = ct_image_name.split('18227')[1][0:2]
                #cb_case = cb_image_name.split('CBCT')[1][0:2]
                cb_case = cb_image_name.split('CB')[1].split('CT')[-1][0:2]
