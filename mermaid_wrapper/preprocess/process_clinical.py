    ...
    def __extract_dicom_files__(self):
        ...
        for folder in os.listdir(self.dicom_root_folder):
            ...
            for f in os.listdir(dicom_case_folder):
                print(f)
                dcmlist = glob.glob(os.path.join(dicom_case_folder,f,'*'))
                f0 = dcmlist[0]
                #f0 = glob.glob(os.path.join(dicom_case_folder, f, '*'))[0]
                cmd = "plastimatch convert --input '{}' " \
                      "--output-img {} " \
                      "--output-prefix {} " \
                      "--prefix-format nii.gz " \
                      "--referenced-ct '{}'".format(f0,
        ...
        if not self.is_special:
            ...
            '''
            print('unzip -qq ' + template_zip_file + ' ' + '-d' + ' ' + self.dicom_root_folder)
            ...
            dicom_template_folder = os.path.join(self.dicom_root_folder, 'template')
...
if __name__ == '__main__':
    ...
    #args = parser.parse_args(['--patients','49','50','51','52','53','96'])
    ...
    #args = parser.parse_args(['--patients','72'])
